<template>
	...
	<div class="personal-wraper">
		...
		<div class="personal-msg-wraper">
			<div class="head">
				<img :src="personMsg.headpic" width="80" height="80" alt="头像" />
			</div>
			<div class="phone">
		...
		<div class="gift-list-wraper distance-wraper">
			...
			<div class="gift-item">
				...
				<div class="coupon-list-wraper" :class="{'gift-detail-on': isUnionpay, 'gift-detail-off': !isUnionpay}">
					<router-link 
			...
			<div class="gift-item">
				...
				<div class="coupon-list-wraper" :class="{'gift-detail-on': isVisa, 'gift-detail-off': !isVisa}">
					<router-link 
			...
			<div class="gift-item">
				...
				<div class="coupon-list-wraper" :class="{'gift-detail-on': isJinnang, 'gift-detail-off': !isJinnang}">
					<router-link 
			...
			<div class="gift-item">
				...
				<div class="coupon-list-wraper" :class="{'gift-detail-on': isGaodaowu, 'gift-detail-off': !isGaodaowu}">
					<router-link 
...
<script lang="ts">
	...
	import { getBaseInfo, showCoupon } from '@/api/personal';
	...
	export default class Personal extends Vue {
		...
		private personMsg: PersonalMsg = { phone: '', headpic: '' };
		...
		private getBaseInfo () {
			try {
				...
				this.personMsg.phone = phone;

				getBaseInfo(id)
					.then(res => {
						const { code, data, message } = res.data;
						if (code === 0) {
							const {personalMsg, couponMsg} = data;
							this.personMsg.headpic = personalMsg[0].user_headpic;
							couponMsg.forEach((coupon: CouponMsg): void => {
		...
		private showCoupon (type: 'union' | 'visa' | 'jinnang' | 'gaodaowu') {
			const { id } = JSON.parse(window.sessionStorage.user_info);
			return showCoupon(id, type);
		}

		private async showUnion () {
			try {
				const res = await this.showCoupon('union');
				const { code, data, message } = res.data;
				if (code === 0) {
		...
		private async showVisa () {
			try {
				const res = await this.showCoupon('visa');
				const { code, data, message } = res.data;
				if (code === 0) {
		...
		private async showJinnang () {
			try {
				const res = await this.showCoupon('jinnang');
				const { code, data, message } = res.data;
				if (code === 0) {
		...
		private async showGaodaowu () {
			try {
				const res = await this.showCoupon('gaodaowu');
				const { code, data, message } = res.data;
				if (code === 0) {
